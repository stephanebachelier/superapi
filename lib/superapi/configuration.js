...
Configuration.prototype.service = function(id) {
  ...
  if (!this.services[id]) {
    var service = new Service(id, this.config.baseUrl, this.config.services[id]);
...
Configuration.prototype.request = function(id, data, params, query) {
  ...
  // add configuration data if provided without overriding possible
  // service data
  var configData = this.config.data;

  for (var name in configData) {
    if (!descriptor.data[name]) {
      descriptor.data[name] = configData[name];
    }
  }
